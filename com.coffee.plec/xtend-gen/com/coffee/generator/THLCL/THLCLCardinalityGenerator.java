...
import com.coffee.generator.THLCL.THLCLCardinalityFactory;
...
public class THLCLCardinalityGenerator extends THLCLGenerator {
  ...
  public THLCLCardinalityGenerator(final String name, final TypeOfProblem type) {
    super(name, type);
    ...
    this.factory = _tHLCLCardinalityFactory;
  }
          ...
          for (final VarDeclaration childVar : _ids) {
            {
              ...
              for (final String nameV : _varsIds) {
                {
                  ...
                  sufixInstances = sufixInstances.substring(0, _minus);
              ...
              if (((!Objects.equal(childNode.getId(), this.root)) && (childNode.getNumInstances() != 1))) {
                ...
                int _minus = (_length - 1);
                allInstances = allInstances.substring(0, _minus);
  ...
  public void processStructural(final Structural exp) {
    final Node padre = this.tree.get(exp.getParent().getName());
    boolean _equals = padre.getId().equals(this.root);
    if (_equals) {
      padre.addVarId(padre.getId());
