...
class PLECGenerator extends AbstractGenerator {
	...
	override void doGenerate(Resource resource, IFileSystemAccess2 fsa, IGeneratorContext context) {
		...
		if(typeOfProblem==TypeOfProblem.SAT){
			//generating an intermediate file with the textual Dimacs representation
			fsa.generateFile(modelName+".cnf", toDIMACS(model, modelName, typeOfProblem))
	...
	def TypeOfProblem setTypeOfProblem(Model model){
		 ...
		 * otherwise is a CSP
		 */ 
		if  (typeOfVariables(model)==TypeOfProblem.CSP_INST){
	...
	def toTHLCL(Model model, String modelName, TypeOfProblem typeOfProblem) {
		...
		switch typeOfProblem{
			 case CSP_INST: thlcl= new THLCLCardinalityGenerator (modelName,  typeOfProblem) //cambiar por instances
			 default:  thlcl= new THLCLGenerator (modelName,  typeOfProblem)
		}
	...
	def toXCSP3(Model model, String modelName,TypeOfProblem typeOfProblem) {
		...
		//se les manda como parámetro el tipo de problema. 
		var XCSP3Generator xcsp3
		switch typeOfProblem{
			 case CSP_INST: xcsp3= new XCSP3CardinalityGenerator (modelName,  typeOfProblem) //cambiar por instances
			 default:  xcsp3= new XCSP3Generator (modelName,  typeOfProblem)
		}
	 ...
	 */
	def toDIMACS(Model model, String modelName,TypeOfProblem typeOfProblem) {
		//en este metodo se debe  instanciar el generador de XCSP3 
		//se les manda como parámetro el tipo de problema. 
	...
	def typeOfVariables(Model model){
		...
		for (variable: model.vars ){
			...
			// then the problem is of type instantiable
		...
		//if the problem contains one integer variable
		// is a CSP
		if (isInteger){
			return TypeOfProblem.CSP
		}
