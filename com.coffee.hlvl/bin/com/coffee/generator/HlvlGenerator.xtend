...
// obtaining the name of the model from the source code
		...
		if(dialect==Dialect.BASIC){
			...
			fsa.generateFile(modelName+"bool.mzn", toBoolean(model, modelName))
		}
		else{
	...
	def booleanRelations(Model model){
		...
		for (rel: model.relations ){
			// if a relation is of type multiinstantions, 
...
//				var int wildCard= -1
	...
	def toBoolean(Model model, String modelName) {
		//en este metodo se debe  instanciar el generador de XCSP3 
		//se les manda como parámetro el tipo de problema. 
		var BoolGenerator boolGen= new BoolGenerator(modelName)
		boolGen.parseModel(model)
	}
...
//		dimacs.parseModel(model)
		var IntGenerator generator= new IntGenerator(modelName, dialect)
